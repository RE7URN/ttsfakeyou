...
app.get("/twitch/callback", async (req, res) => {
  ...
  if (!code) {
    return res.send("❌ Código no proporcionado.");
  }
  ...
  try {
    ...
    return res.send("✅ Token de Twitch recibido. Ya puedes cerrar esta pestaña.");
  } catch (err) {
    console.error("❌ Error al obtener token de usuario:", err.response?.data || err.message);
    return res.send("❌ Error al obtener token de usuario");
  }
});
